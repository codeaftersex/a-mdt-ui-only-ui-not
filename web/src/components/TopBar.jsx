...
export function TopBar() {
  ...
  return (
    <header className="flex h-14 items-center gap-4 border-b border-white/5 bg-[#0b1220]/60 px-6 backdrop-blur">
      <div className="relative flex-1 max-w-xl">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
        <input
          placeholder="Search name, license, plate, report #…"
          className="w-full rounded-md border border-white/5 bg-white/[0.03] py-2 pl-10 pr-4 text-sm text-slate-200 placeholder:text-slate-500 outline-none focus:border-sky-500/50 focus:bg-white/[0.05] transition-colors"
        />
      </div>
      ...
      <div className="flex items-center gap-3 border-l border-white/5 pl-4">
        <button className="relative grid h-9 w-9 place-items-center rounded-md border border-white/5 bg-white/[0.03] hover:bg-white/[0.06] transition-colors">
        ...
        <div className="flex items-center gap-2.5 rounded-md border border-white/5 bg-white/[0.03] px-3 py-1.5">
          <Avatar
            src={officer.photo}
            fallback="JR"
            alt={officer.name}
